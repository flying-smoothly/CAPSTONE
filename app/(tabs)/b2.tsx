...
const IMG = require('../../assets/images/B2(문자X, 아이콘 삽입).png');
...
const raw: [number, number][] = [
  ...
  [2,1], [6,1], [10,1], [14,1], [18,1], [18,3], [22,1], [26,1], [30,1],
...
// 네가 쓰던 B2 기본 보정값
const CAL_INIT = { x0: -34.0, x1: 51.0, y0: -55.0, y1: 32.5 };
...
export default function b2Screen() {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => console.log('[WS B2] connected');
    ws.onmessage = (e) => {
      // 모든 메시지 공통 로그
      console.log('[WS B2] msg:', e.data);
      let msg: any;
      try {
      ...
      // 1) 위치/경로 업데이트(이전과 동일)
      if (String(msg.floor || '').toUpperCase() === 'B2') {
      ...
      // 2) 🔥 화재 알림 받기
      if (msg?.kind === 'fire_alert' && String(msg.floor || '').toUpperCase() === 'B2') {
        const now = Date.now();
        ...
        // 간단 경고창도 함께(원하면 주석)
        if (Platform.OS !== 'web') {
          Alert.alert('화재 감지', `층: B2\n신뢰도: ${alertMsg.confidence.toFixed(2)}`);
        }
      }
    };
    ws.onclose = () => console.log('[WS B2] closed');
    ws.onerror = (err) => console.warn('[WS B2] error', err);
    return () => ws.close();
  }, []);
      ...
      {fireAlert && (
        <View style={styles.banner}>
          <Text style={styles.bannerTxt}>
            🔥 화재 감지(B2) conf={fireAlert.confidence.toFixed(2)}
          </Text>
