...
// 예: components/SafetyOverlay.tsx, app/(tabs)/b1.tsx 등

// 화면 배경도면 (파일명에 괄호가 있어도 동작은 합니다)
const IMG = require('../../assets/images/B1(문자X, 아이콘 삽입).png');
...
const raw: [number, number][] = [
  ...
  [-2,-3], [-2,1], [-2,5], [2,1], [6,1], [10,1], [14,1], [14,13], [18,1], [18,5],
  [18,9], [18,13], [18,17], [22,5], [22,9], [26,5], [26,9],
];
const POINTS = toPts(raw);

// 화면 보정 초기값
const CAL_INIT = { x0: -36.5, x1: 49.0, y0: -48.5, y1: 35.5 };

// 안내 이미지 경로(문자열/require 모두 가능)
// 예) const FIRE_MANUAL_URI = require('../../assets/safety/manual.jpg');
// 화재시 대피방법
...
export default function B1Screen() {
  ...
  const [hazards, setHazards] = useState<Array<[number, number]>>([]);

  useEffect(() => {
    const ws = new WebSocket(WS_URL);
    ws.onopen = () => console.log('[WS B1] connected');
    ws.onmessage = (e) => {
      // 모든 메시지 공통 로그
      console.log('[WS B1] msg:', e.data);
      let msg: any;
      try {
      ...
      // 1) 위치/경로 업데이트(이전과 동일)
      if (String(msg.floor || '').toUpperCase() === 'B1') {
      ...
      // 2) 🔥 화재 알림 받기
      if (msg?.kind === 'fire_alert' && String(msg.floor || '').toUpperCase() === 'B1') {
        const now = Date.now();
        ...
        // 간단 경고창도 함께(원하면 주석)
        if (Platform.OS !== 'web') {
          Alert.alert('화재 감지', `층: B1\n신뢰도: ${alertMsg.confidence.toFixed(2)}`);
        }
      }

    // 3) 화재 위치 갱신
    if (msg.kind === 'hazard' && Array.isArray(msg.node) && msg.node.length === 2) {
      const xy: [number,number] = [+msg.node[0], +msg.node[1]];
    ...
    ws.onclose = () => console.log('[WS B1] closed');
    ws.onerror = (err) => console.warn('[WS B1] error', err);
    return () => ws.close();
  }, []);
      ...
      {fireAlert && (
        <View style={styles.banner}>
          <Text style={styles.bannerTxt}>
            🔥 화재 감지(B1) conf={fireAlert.confidence.toFixed(2)}
          </Text>
