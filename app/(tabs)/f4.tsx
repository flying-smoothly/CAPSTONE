...
export default function F4Screen() {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => console.log('[WS 4F] connected');
    ws.onmessage = (e) => {
      // 모든 메시지 공통 로그
      console.log('[WS 4F] msg:', e.data);
      let msg: any;
      try {
      ...
      if (msg?.kind === 'fire_alert' && String(msg.floor || '').toUpperCase() === '4F') {
        ...
        // 간단 경고창도 함께(원하면 주석)
        if (Platform.OS !== 'web') {
          Alert.alert('화재 감지', `층: 4F\n신뢰도: ${alertMsg.confidence.toFixed(2)}`);
        }
      }
    };
    ws.onclose = () => console.log('[WS 4F] closed');
    ws.onerror = (err) => console.warn('[WS 4F] error', err);
    return () => ws.close();
  }, []);
